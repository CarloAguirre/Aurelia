...
export function InspectionChatScreen() {
  ...
  function renderMessage(msg: MessageItem) {
    ...
    switch (msg.type) {
      ...
      case 'error':
        return <ErrorBubble key={msg.id} message={msg.message} onRetry={retryCallbacks.current.get(msg.id)} />;
      case 'photo_widget':
        return <PhotoStepWidget key={msg.id} onSkip={() => handlePhotoSkip(msg.id)} onCapture={(uri) => handlePhotoCapture(uri, msg.id)} resolved={isResolved} />;
      case 'area_chips':
        return <ChipRow key={msg.id} chips={msg.areas.map((area) => area.name)} selected={isResolved ? flow.areaName : null} onSelect={(name) => { if (!isResolved) { const area = msg.areas.find((item) => item.name === name); if (area) void handleAreaSelect(area, msg.id); } }} />;
      case 'sector_chips':
      ...
      case 'prob_chips':
        return <ChipRow key={msg.id} chips={PROBS} selected={isResolved && flow.currentObs.prob ? PROBS[flow.currentObs.prob - 1] : null} onSelect={(label) => { if (!isResolved) void handleProbSelect(label, msg.id); }} />;
      case 'cons_chips':
        return <ChipRow key={msg.id} chips={CONS} selected={isResolved && flow.currentObs.cons ? CONS[flow.currentObs.cons - 1] : null} onSelect={(label) => { if (!isResolved) void handleConsSelect(label, msg.id); }} />;
      case 'more_obs_opts':
        return <QuickOpts key={msg.id} options={[{ value: 'mas', label: 'Sí, agregar otra' }, { value: 'empresa', label: 'No, pasar a empresa y personal' }]} selected={isResolved ? 'empresa' : null} onSelect={(value) => { if (!isResolved) void handleMoreObs(value, msg.id); }} />;
      case 'company_chips':
        return <ChipRow key={msg.id} chips={msg.companies.map((company) => company.name)} selected={isResolved ? flow.companyName : null} onSelect={(name) => { if (!isResolved) { const company = msg.companies.find((item) => item.name === name); if (company) void handleCompanySelect(company, msg.id); } }} />;
      case 'personnel_picker':
        return <PersonnelPicker key={msg.id} users={msg.users} confirmed={confirmedPickerIds.has(msg.id)} onConfirm={(selected) => handlePersonnelConfirm(selected, msg.id)} />;
      case 'submit_btn':
        return <SubmitWidget key={msg.id} obsCount={msg.obsCount} photoCount={msg.photoCount} submitted={isResolved || saveAssistantInspection.isPending} onSubmit={() => handleSubmit(msg.id)} />;
      default:
        return null;
    }
  }
