...
export function InspectionChatScreenV2() {
  ...
  async function askTemplate() { push('typing'); const r = await templatesQuery.refetch(); clearTyping(); const list = r.data ?? templates; if (!list.length) { err('No hay plantillas disponibles.'); return; } push('bot', 'Te sugiero esta plantilla normativa.'); push('templatePick', { template: list[0], templates: list }); }
  ...
  async function generalPhoto(uri: string, mid: string) { resolve(mid); draft.setGeneralPhoto(asset(uri, 'foto_general.jpg')); push('user', 'Foto general registrada'); await sleep(200); const first = itemsOf(template)[0]; if (!first) { err('La plantilla no tiene ítems.'); return; } push('bot', `Responderemos ${rows.length} ítems.`); push('question', first); }
  ...
  async function submit(mid: string) { resolve(mid); const st = useManualInspectionDraft.getState(); const t = templates.find((x) => x.id === st.templateId); if (!t) { err('Falta plantilla.'); return; } push('typing'); try { const result = await save.mutateAsync({ draft: st, template: t, items: itemsOf(t), trySyncNow: online && hasSession }); st.setLastSavedResult?.(result); clearTyping(); router.replace({ pathname: '/inspection/success', params: { inspectionId: result.inspectionId, findingsCount: String(result.noCount), evidencesCount: String(1 + result.noCount), areaName: st.areaName ?? '', sectorName: st.sectorName ?? '', companyName: st.findingCompanyName ?? st.inspectorCompanyName, personnelNames: st.findingResponsibleIds.length ? `${st.findingResponsibleIds.length} responsables seleccionados` : '', criticalCount: '0' } }); } catch { clearTyping(); err('Error al guardar.', () => submit(mid)); } }
