...
function buildActiveFilters(filters: TableFilters): ActiveFilter[] {
  ...
  if (filters.obs === 'rejected') chips.push({ key: 'obs', label: 'Obs.: rechazadas' });
...
function buildHistoryKpis(data: InspectionHistoryKpisResponse | undefined, isLoading: boolean, isError: boolean) {
  const currentYear = new Date().getFullYear();
  if (isLoading) return { year: currentYear, closedInspections: '...', averageClosureDays: '...', closedFindingsRate: '...', contractorCompanies: '...' };
  if (isError || !data) return { year: currentYear, closedInspections: '—', averageClosureDays: '—', closedFindingsRate: '—', contractorCompanies: '—' };
  return {
    year: data.year,
...
function sortableValue(row: Row, key: SortKey) {
  ...
  if (key === 'closure') return row.closure;
...
function KpiIcon({ kind }: { kind: 'closed' | 'average' | 'rate' | 'company' }) {
  const color = kind === 'closed' || kind === 'rate' ? '#53bd49' : kind === 'average' ? '#24588b' : '#24588b';
  if (kind === 'average') return <svg className="h-[11px] w-[13.75px] shrink-0" fill="none" viewBox="0 0 14 11" aria-hidden><rect x="2" y="2" width="10" height="7" rx="1.5" stroke={color} strokeWidth="1.4" /><path d="M4 1v2M10 1v2M4 5.3h6" stroke={color} strokeWidth="1.2" strokeLinecap="round" /></svg>;
  if (kind === 'rate') return <svg className="h-[11px] w-[13.75px] shrink-0" fill="none" viewBox="0 0 14 11" aria-hidden><path d="M2 8.5l3-3 2 1.8 4-4.8" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /><path d="M10.8 2.5H12v1.3" stroke={color} strokeWidth="1.5" strokeLinecap="round" /></svg>;
  if (kind === 'company') return <svg className="h-[11px] w-[13.75px] shrink-0" fill="none" viewBox="0 0 14 11" aria-hidden><rect x="2" y="3" width="4" height="6" rx="1" stroke={color} strokeWidth="1.3" /><rect x="7" y="1.5" width="5" height="7.5" rx="1" stroke={color} strokeWidth="1.3" /><path d="M3.4 5h1.2M8.4 3.5h2.2M8.4 5.5h2.2" stroke={color} strokeWidth="1" strokeLinecap="round" /></svg>;
  return <svg className="h-[11px] w-[13.75px] shrink-0" fill="none" viewBox="0 0 14 11" aria-hidden><circle cx="6.875" cy="5.5" r="4.85" fill={color} /><path d="M4.7 5.55l1.35 1.35 2.95-3.1" stroke="white" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" /></svg>;
}
...
function KpiCard({ icon, label, value, helper }: { icon: 'closed' | 'average' | 'rate' | 'company'; label: string; value: string; helper: string }) {
  return (
    <div className="flex h-[92.5px] min-w-0 flex-col items-start rounded-[8px] border border-[#e3e3e3] bg-white px-[17px] py-[15px] drop-shadow-[0px_1px_1.5px_rgba(0,0,0,0.05)]">
      <div className="flex h-[14px] w-full items-center gap-[6px]"><KpiIcon kind={icon} /><p className="whitespace-nowrap font-['Inter:Semi_Bold',sans-serif] text-[11px] font-semibold uppercase leading-[normal] tracking-[0.44px] text-[#646464]">{label}</p></div>
...
function ActiveFiltersBar({ filters, onRemove }: { filters: ActiveFilter[]; onRemove: (key: TableFilterKey) => void }) {
  ...
  return <div className="flex min-h-[38px] w-full flex-wrap items-center justify-between gap-[12px] pt-[16px]"><div className="flex min-h-[38px] min-w-[280px] flex-1 items-center gap-[10px] overflow-x-auto bg-[#eef5ff] px-[14px] py-[10px]"><div className="flex shrink-0 items-center gap-[6px]"><span className="font-['Inter:Semi_Bold',sans-serif] text-[11px] font-semibold leading-[13px] text-[#24588b]">Filtros activos:</span></div>{filters.map((filter) => <ActiveFilterChip key={filter.key} label={filter.label} onRemove={() => onRemove(filter.key)} />)}</div><TableActions /></div>;
}
...
function DataCell({ children, center = false, bold = false }: { children: ReactNode; center?: boolean; bold?: boolean }) {
  return <td className={`border-r border-b border-[#e3e3e3] bg-white px-[12px] py-[13.5px] align-middle text-[12px] leading-[normal] text-[#333] ${center ? 'text-center' : 'text-left'} ${bold ? "font-['Inter:Semi_Bold',sans-serif] font-semibold text-[#131313]" : "font-['Inter:Regular',sans-serif] font-normal"}`}>{children}</td>;
}
...
function DaysCell({ value }: { value: number }) {
  return <td className="border-r border-b border-[#e3e3e3] bg-white px-[12px] py-[13.5px] text-center align-middle font-['Inter:Bold',sans-serif] text-[12px] font-bold leading-[normal] text-[#2a5c16]">{value}</td>;
}

function ProgressCell({ value }: { value: number }) {
  return <td className="border-r border-b border-[#e3e3e3] bg-white px-[12px] py-[14px] align-middle"><div className="flex w-[95px] items-center gap-[5px]"><div className="h-[4px] min-w-[36px] flex-[62_0_0] rounded-[2px] bg-[#e3e3e3]"><div className="h-[4px] rounded-[2px] bg-[#53bd49]" style={{ width: `${Math.max(0, Math.min(100, value))}%` }} /></div><span className="min-w-[28px] text-right font-['Inter:Bold',sans-serif] text-[10px] font-bold leading-[normal] text-[#2a5c16]">{formatPercent(value)}</span></div></td>;
}
...
function getObsBadge(item: string) {
  ...
  if (item.includes('Rech')) return { tone: 'pink' as const, icon: 'alert' as const };
        ...
        <table className="table-fixed border-collapse" style={{ minWidth: `${tableWidth}px`, width: `${tableWidth}px` }}>
          ...
          <thead>
            ...
            <tr className="h-[37px] bg-[#f0f4f8]">
              ...
              <FilterCell><TableTextFilter value={filters.date} onChange={(value) => onFilterChange('date', value)} width={123} placeholder="dd-mm-aaaa" /></FilterCell>
          ...
          <tbody>
            {sortedRows.map((row) => (
              <tr key={row.uniqueKey} style={{ height: `${row.height}px` }}>
                ...
                <DataCell><Badge tone={row.urgencyTone} icon={row.urgency.includes('Cerrada') ? 'check' : row.urgency.includes('Crítico') ? 'alert' : 'clock'}>{row.urgency}</Badge></DataCell>
            ...
            {!isLoading && sortedRows.length === 0 ? <tr className="h-[61px]"><td colSpan={12} className="border-b border-[#e3e3e3] bg-white px-[12px] py-[13.5px] text-center font-['Inter:Regular',sans-serif] text-[12px] text-[#646464]">No hay inspecciones cerradas para mostrar</td></tr> : null}
          </tbody>
        </table>
      </div>
...
export function InspectionsHistoryView() {
  ...
  const kpis = buildHistoryKpis(kpisQuery.data, kpisQuery.isLoading, kpisQuery.isError);
  ...
  return (
    <>
      <div className="flex h-[calc(100vh-56px)] w-full flex-col items-start overflow-x-hidden overflow-y-auto bg-[#f7f7f7] px-[24px] py-[20px]">
        <div className="grid w-full grid-cols-[repeat(auto-fit,minmax(244px,1fr))] gap-[12px]"><KpiCard icon="closed" label="Inspecciones cerradas" value={kpis.closedInspections} helper={`Año ${kpis.year}`} /><KpiCard icon="average" label="Promedio de cierre" value={kpis.averageClosureDays} helper="días por inspección" /><KpiCard icon="rate" label="% Obs. cerradas" value={kpis.closedFindingsRate} helper="sobre inspecciones cerradas" /><KpiCard icon="company" label="Empresas EECC" value={kpis.contractorCompanies} helper="empresas con al menos 1 insp." /></div>
        <ActiveFiltersBar filters={activeFilters} onRemove={removeFilter} />
        <div className="w-full pt-[16px]"><InspectionHistoryTable rows={tableRows} total={total} page={tableQuery.data?.page ?? page} totalPages={totalPages} pageSize={pageSize} isLoading={tableQuery.isLoading} isError={tableQuery.isError} filters={filters} options={filterOptions} sort={sort} onSort={handleSort} onFilterChange={updateFilter} onClearFilters={clearFilters} onPageChange={setPage} onPageSizeChange={changePageSize} onViewDetails={openInspectionDetail} /></div>
      </div>
