  ...
  return (
    <div className="ml-[33px] overflow-hidden rounded-[12px] border border-[#E3E3E3] bg-white">
      ...
      <div className="border-t border-[#E3E3E3] px-[12px] py-[10px]">
        ...
        {suggestion && !editing ? <div className="overflow-hidden rounded-[10px] border-[1.5px] border-[#C8A064] bg-white"><div className="flex items-center justify-between gap-[6px] border-b border-[rgba(200,160,100,0.2)] bg-gradient-to-br from-[#FDF3E3] to-[#FAE8C8] px-[12px] py-[7px]"><div className="flex items-center gap-[6px]"><span className="text-[10px] text-[#8E6E3E]">✦</span><span className="text-[10px] font-bold text-[#8E6E3E]">Acción sugerida por AurelIA</span></div><span className="rounded-[5px] bg-white px-[6px] py-[2px] text-[8px] font-bold uppercase leading-none text-[#8E6E3E]">Mock</span></div><div className="px-[12px] py-[10px]"><p className="text-[12px] font-medium leading-[18px] text-[#131313]">{suggestion}</p></div><div className="flex gap-[6px] px-[12px] pb-[10px]"><button type="button" onClick={onEdit} className="flex h-[34px] items-center justify-center rounded-[8px] border-[1.5px] border-[#D1D1D1] bg-white px-[12px] text-[12px] font-semibold text-[#333]">Editar</button><button type="button" onClick={onAccept} className="flex h-[34px] flex-1 items-center justify-center rounded-[8px] bg-[#00B398] text-[12px] font-bold text-white">Aceptar</button></div></div> : null}
    ...
    <div className="absolute inset-0 z-40 flex flex-col overflow-hidden bg-[#F4F6F9]">
      ...
      <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden bg-[#F4F6F9] px-[12px] py-[12px]">
        <div className="flex flex-col gap-[10px]">
          ...
          {phase === 'response' ? <><AgentBubble>Perfecto. Completa tu respuesta: sube la foto <strong>Después</strong> y describe la acción que tomaste.</AgentBubble><ResponseCard item={item} file={file} suggestion={suggestion} editing={editing} description={description} onFile={handleFile} onAccept={acceptSuggestion} onEdit={() => setEditing(true)} onDescriptionChange={setDescription} onSaveDescription={saveDescription} />{loadingSuggestion ? <TypingDots /> : null}{canSummarize ? <QuickOption tone="teal" onClick={() => setPhase('summary')}>→ Continuar al resumen</QuickOption> : null}</> : null}
