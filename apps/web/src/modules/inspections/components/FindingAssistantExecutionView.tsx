...
function FindingCard({ item, index }: { item: InspectionDetailFindingItemResponse | null | undefined; index: number }) {
  ...
  return <div className="ml-[33px] overflow-hidden rounded-[12px] border border-[#E3E3E3] bg-white shadow-[0_1px_4px_rgba(0,0,0,0.06)]"><div className="flex items-center justify-between bg-[#001E39] px-[14px] py-[9px]"><span className="min-w-0 truncate text-[11px] font-bold text-white">Obs. {index + 1}</span><FindingBadge className={severityClassName(item?.severityLabel)}>{item?.severityLabel ?? 'Moderado'}</FindingBadge></div><div className="relative flex h-[110px] flex-col items-center justify-center gap-[6px] overflow-hidden border-b border-[#E3E3E3] bg-gradient-to-br from-[#E8F4FD] to-[#C8E6F0]">{beforeUrl ? <img src={beforeUrl} alt={evidenceLabel(beforeEvidence, `Foto Antes · Obs. ${index + 1}`)} className="h-full w-full object-cover" /> : <><span className="text-[28px] text-[#24588B]">▧</span><span className="text-[10px] text-[#646464]">Foto Antes · Obs. {index + 1}</span></>}<div className="absolute left-[10px] top-[8px] rounded-[4px] bg-[rgba(0,0,0,0.55)] px-[8px] py-[3px]"><p className="text-[9px] font-bold uppercase tracking-[1.5px] text-white">Foto antes</p></div>{beforeEvidence?.capturedAt ? <div className="absolute bottom-[8px] right-[10px] rounded-[4px] bg-[rgba(0,0,0,0.5)] px-[7px] py-[3px]"><p className="text-[9px] leading-none text-[rgba(255,255,255,0.85)]">{formatDateTime(beforeEvidence.capturedAt)}</p></div> : null}</div><div className="flex flex-col gap-[7px] px-[12px] py-[10px]"><div className="flex gap-[8px]"><p className="min-w-[72px] pt-px text-[9px] font-bold uppercase tracking-[0.05em] text-[#646464]">Condición</p><p className="flex-1 text-[12px] font-medium leading-[16.8px] text-[#131313]">{item?.condition ?? '—'}</p></div><div className="flex gap-[8px]"><p className="min-w-[72px] pt-px text-[9px] font-bold uppercase tracking-[0.05em] text-[#646464]">Medida solicitada</p><p className="flex-1 text-[12px] font-medium leading-[16.8px] text-[#131313]">{item?.proposedCorrectiveAction ?? '—'}</p></div></div></div>;
}
