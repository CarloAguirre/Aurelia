...
export function FindingManualExecutionView({ subtitle, item, index = 1, isSubmitting = false, onBack, onCancel, onSubmit }: { subtitle: string; item?: InspectionDetailFindingItemResponse | null; index?: number; isSubmitting?: boolean; onBack: () => void; onCancel: () => void; onSubmit: (description: string, file: File) => void | Promise<void> }) {
  ...
  if (step === 2) {
    return (
      <div className="absolute inset-0 z-40 flex flex-col overflow-hidden bg-[#f7f7f7]">
        <Header subtitle={subtitle} onBack={handleBack} />
        <Stepper step={2} />
        <div className="min-h-0 flex-1 overflow-y-auto bg-[#f7f7f7] px-[14px] pt-[14px]">
          <section className="overflow-hidden rounded-[12px] border border-[#e3e3e3] bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            <div className="flex h-[29px] items-center gap-[6px] border-b border-[#e3e3e3] bg-[#f7f7f7] px-[12px]"><span className="text-[#646464]"><ResponsibleIcon /></span><p className="text-[10px] font-bold uppercase leading-none tracking-[0.5px] text-[#646464]">Responsables</p></div>
            <div className="flex items-center justify-between border-b border-[#e3e3e3] px-[12px] py-[9px]"><p className="text-[12px] font-normal leading-none text-[#646464]">Fecha ejecución</p><p className="text-right text-[12px] font-bold leading-none text-[#131313]">{formatDate(new Date().toISOString())}</p></div>
            <div className="flex items-center justify-between border-b border-[#e3e3e3] px-[12px] py-[9px]"><p className="text-[12px] font-medium leading-none text-[#646464]">EECC</p><p className="max-w-[180px] truncate text-right text-[11px] font-bold leading-none text-[#131313]">{responsibleCompany}</p></div>
            <div className="flex items-center gap-[10px] px-[12px] py-[10px]"><div className="flex size-[32px] shrink-0 items-center justify-center rounded-[16px] bg-[#c8a064] text-[12px] font-bold leading-none text-[#001e39]">{initials(responsibleName)}</div><div className="min-w-0 flex-1"><p className="truncate text-[12px] font-bold leading-none text-[#131313]">{responsibleName}</p><p className="pt-[4px] text-[11px] font-normal leading-none text-[#646464]">{responsiblePosition}</p></div>{responsible?.currentUser ? <span className="inline-flex h-[16px] items-center rounded-[5px] bg-[#c5fff6] px-[7px] text-[10px] font-bold leading-none text-[#00b398]">Tú</span> : null}</div>
          </section>
          <section className="mt-[12px] overflow-hidden rounded-[12px] border border-[#e3e3e3] bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            <div className="flex h-[29px] items-center gap-[6px] border-b border-[#e3e3e3] bg-[#f7f7f7] px-[12px]"><span className="text-[#646464]"><ObservationIcon /></span><p className="text-[10px] font-bold uppercase leading-none tracking-[0.5px] text-[#646464]">Observación respondida</p></div>
            <div className="flex flex-col gap-[8px] px-[12px] pb-[10px] pt-[9px]"><div className="flex items-center justify-between"><div className="flex items-center gap-[8px]"><FindingPill className="bg-[#e6f3ff] text-[#24588b]">{`Obs. ${index + 1}`}</FindingPill><FindingPill className={severityClassName(severityLabel)}>{severityLabel}</FindingPill></div><span className="inline-flex items-center rounded-[99px] bg-[#e0ffd3] px-[10px] py-[4px] text-[10px] font-bold uppercase leading-none tracking-[1px] text-[#2a5c16]">Respondida</span></div><p className="text-[12px] font-normal leading-[16.8px] text-[#131313]">{condition}</p></div>
          </section>
          <div className="mt-[12px] rounded-[10px] bg-[#e6f3ff] px-[11px] py-[9px]"><p className="text-[12px] font-normal leading-[18px] text-[#0d3862]">Al enviar, el Admin GF HSE recibirá una alerta para validar cada observación. Solo el Admin GF puede cambiar el estado a Cerrado definitivamente.</p></div>
        </div>
        <div className="shrink-0 border-t border-[#e3e3e3] bg-white pb-[8px] pt-[10px]"><div className="flex gap-[10px] px-[14px]"><button type="button" className="flex h-[50px] items-center justify-center gap-[8px] rounded-[14px] border-2 border-[#c8a064] bg-white px-[20px] text-[14px] font-bold text-[#c8a064]" onClick={() => setStep(1)} disabled={isSubmitting}><ArrowLeftIcon />Atrás</button><button type="button" className="flex h-[50px] min-w-0 flex-1 items-center justify-center gap-[8px] rounded-[14px] bg-[#3a9b3a] px-[12px] text-[14px] font-bold text-white" onClick={submit} disabled={isSubmitting}>{isSubmitting ? 'Enviando...' : 'Enviar al ADMIN GF'} <ArrowRightIcon /></button></div><div className="mx-auto mb-[4px] mt-[12px] h-[4px] w-[120px] rounded-[2px] bg-[#d1d1d1]" /></div>
      </div>
    );
  }
  ...
  return (
    <div className="absolute inset-0 z-40 flex flex-col overflow-hidden bg-[#f7f7f7]">
      ...
      <div className="shrink-0 border-t border-[#e3e3e3] bg-white pb-[8px] pt-[10px]"><div className="flex gap-[10px] px-[14px]"><button type="button" className="flex h-[50px] items-center justify-center rounded-[14px] border-2 border-[#c8a064] bg-white px-[20px] text-[14px] font-bold text-[#c8a064]" onClick={onCancel} disabled={isSubmitting}>Cancelar</button><button type="button" className={`flex h-[50px] min-w-0 flex-1 items-center justify-center gap-[8px] rounded-[14px] px-[12px] text-[14px] font-bold ${canAdvance ? 'bg-[#c8a064] text-[#001e39]' : 'bg-[#d1d1d1] text-[#acacac]'}`} disabled={!canAdvance} onClick={() => setStep(2)}>Marcar como ejecutado <ArrowRightIcon /></button></div><div className="mx-auto mb-[4px] mt-[12px] h-[4px] w-[120px] rounded-[2px] bg-[#d1d1d1]" /></div>
    </div>
  );
}
