...
function StatusDot({ status }: { status: StatusKey }) {
  ...
  if (status === 'executed' || status === 'rejected') {
    return <span className={`relative inline-flex h-[11px] w-[13.75px] items-center justify-center rounded-full ${config.dotClass}`}><span className="h-[1.5px] w-[1.5px] rounded-full bg-white" /><span className="absolute h-[5px] w-[1.5px] -translate-y-[1px] rounded-full bg-white" /></span>;
  }
  return <span className={`relative inline-flex h-[11px] w-[13.75px] items-center justify-center rounded-full ${config.dotClass}`}><span className="h-[4.4px] w-[1.3px] -translate-y-[1px] rounded-full bg-white" /><span className="absolute h-[1.3px] w-[4px] translate-x-[1px] translate-y-[1.3px] rounded-full bg-white" /></span>;
}
