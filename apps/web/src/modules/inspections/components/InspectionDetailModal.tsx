...
function EvidencePreview({ title, tone, iconTone }: { title: string; tone: string; iconTone: string }) {
  return (
    <div className="flex h-[91px] min-w-0 flex-1 flex-col overflow-hidden rounded-[6px] border border-[#e3e3e3] bg-white p-px">
      ...
      <div className={`flex min-h-0 flex-1 items-center justify-center ${tone}`}><InspectionDetailImageIcon tone={iconTone} /></div>
    </div>
  );
}

function FindingObservationCard({ status }: { status: StatusKey }) {
  const config = statusConfigByKey[status];
  return (
    <div className="shrink-0 bg-white px-[14px] pb-[24px] pt-[14px]">
      <div className="rounded-[10px] border-[1.5px] border-[#e3e3e3] bg-[#f7f7f7] p-[13.5px] shadow-[0px_1px_1.5px_rgba(0,0,0,0.06)]">
        ...
        <div className="flex flex-col gap-[4px] pt-[12px]">
          ...
          <div className="flex gap-[4px] pt-[8px]"><EvidencePreview title="Antes" tone="bg-gradient-to-br from-[#e8f4fd] to-[#c8e6f0]" iconTone="#24588b" /><EvidencePreview title="Después" tone="bg-[#dafccb]" iconTone="#2a5c16" /></div>
  ...
  return (
    <div className="min-h-0 flex-1 overflow-y-auto bg-white">
      {statusConfigs.map((config) => {
        ...
        return <div key={config.key}><StatusRow config={config} count={counts[config.key]} expanded={expanded} onToggle={() => setExpandedStatus((current) => current === config.key ? null : config.key)} />{kind === 'finding' && config.key === 'open' && expanded ? <FindingObservationCard status="open" /> : null}</div>;
      })}
    </div>
  );
}
