...
function timelineItem(title: string, date: string, body: string, completed: boolean, isLast = false) {
  return `<div class="relative w-full"><div class="relative flex w-full gap-[12px] items-start ${isLast ? '' : 'pb-[16px]'}"><div class="relative shrink-0 size-[24px]"></div><div class="min-w-0 flex-1 pt-[2px]"><p class="text-[12px] font-bold leading-none text-[#131313]">${escapeHtml(title)}</p><p class="pt-[4px] text-[11px] font-normal leading-none text-[#646464]">${escapeHtml(date)}</p>${body}</div>${isLast ? '' : '<div class="absolute left-[11px] top-[24px] h-[38px] w-[2px] bg-[#e3e3e3]"></div>'}${checkMarker(completed)}</div></div>`;
}
...
function renderFollowups(panel: HTMLElement, counts: FindingCounts) {
  ...
  const closedPercent = percent(counts.closed, total);
  const pendingPercent = percent(pending, total);
  ...
  const followupDate = panel.dataset.aureliaFollowupDate || dates[1] || '—';
  const firstCompleted = counts.closed > 0 || counts.executed > 0 || counts.rejected > 0;
  const signature = `${counts.executed}|${counts.open}|${counts.closed}|${counts.rejected}|${initialDate}|${followupDate}`;
  ...
  const firstBody = `<div class="pt-[2px]"><ul class="list-disc text-[11px] font-normal leading-normal text-[#646464]"><li class="ms-[16.5px]">Observaciones cerradas: ${counts.closed} obs / ${closedPercent}%</li><li class="ms-[16.5px]">Observaciones pendientes: ${pending} obs / ${pendingPercent}%</li></ul></div>`;
  panel.innerHTML = `<div class="flex items-center gap-[6px]"><svg class="h-[11px] w-[13.75px]" width="14" height="11" viewBox="0 0 14 11" fill="none" aria-hidden="true"><circle cx="3" cy="2.25" r="2" fill="#24588B"/><circle cx="11" cy="2.25" r="2" fill="#24588B"/><circle cx="7" cy="8.5" r="2" fill="#24588B"/><path d="M3 4.25V5.5C3 6.05 3.45 6.5 4 6.5H6.05M11 4.25V5.5C11 6.05 10.55 6.5 10 6.5H7.95M5 2.25H9" stroke="#24588B" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round"/></svg><p class="text-[11px] font-bold uppercase leading-none tracking-[0.55px] text-[#646464]">Historial de seguimientos</p></div><div class="pt-[10px]">${timelineItem('Inspección inicial', initialDate, `<p class="pt-[5px] text-[11px] font-normal leading-none text-[#646464]">${total} ${obsWord(total)} detectadas</p>`, true)}${timelineItem('Seguimiento 1', firstCompleted ? followupDate : '—', firstCompleted ? firstBody : '', firstCompleted)}${timelineItem('Seguimiento 2', '—', '', false)}${timelineItem('Seguimiento 3', '—', '', false, true)}</div>`;
}
