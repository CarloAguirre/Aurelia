...
import { type NewInspectionPickedAsset, useNewInspectionDraftStore } from '../state/newInspectionDraft.store';
...
type MsgType = 'bot' | 'user' | 'typing' | 'error' | 'areas' | 'sectors' | 'types' | 'dates' | 'loc' | 'templatePick' | 'templates' | 'generalPhoto' | 'question' | 'itemPhoto' | 'findingTypes' | 'findingPhoto' | 'criticality' | 'sla' | 'findingNext' | 'companies' | 'people' | 'summary';
...
function rowsOf(template: InspectionChecklistTemplateResponse | null): Row[] {
  ...
  return template.sections
    ...
    .flatMap((section) => section.items.slice().sort((left, right) => left.sortOrder - right.sortOrder).map((item) => ({ ...item, sectionTitle: section.title, index: 0 })))
  ...
  return (
    <div className="mb-[10px] ml-[33px] flex flex-wrap gap-[6px]">
      {options.map((option) => {
        ...
        return <button key={option.value} type="button" onClick={() => onSelect(option.value)} className={`rounded-[9999px] border-[1.5px] px-[14px] py-[7px] text-[12px] font-semibold ${active ? 'border-[#002659] bg-[#002659] text-white' : 'border-[#D1D1D1] bg-white text-[#24588B]'}`}>{option.icon ? `${option.icon === 'check' ? '✓ ' : option.icon === 'plus' ? '+ ' : option.icon === 'list' ? '☰ ' : ''}` : ''}{option.label}</button>;
      })}
    </div>
  );
}
...
function LocationWidget({ captured, label, accuracy, capturing, resolved, onCapture }: { captured: boolean; label: string; accuracy: string; capturing: boolean; resolved: boolean; onCapture: () => void }) {
  return (
    <div className="mb-[10px] ml-[33px] mr-[12px] rounded-[12px] border border-[#E3E3E3] bg-white p-[12px]">
      ...
      <button type="button" className={`mt-[8px] h-[44px] w-full rounded-[10px] text-[12px] font-bold text-white ${captured ? 'bg-[#3A9B3A]' : 'bg-[#C8A064]'}`} onClick={onCapture} disabled={capturing || resolved}>{capturing ? 'Capturando ubicación...' : captured ? 'Ubicación capturada' : 'Capturar ubicación'}</button>
      <div className="mt-[8px] rounded-[8px] border border-[#E3E3E3] bg-[#F4F6F9] px-[12px] py-[8px]"><p className="text-[11px] font-semibold text-[#131313]">{label}</p><p className="mt-[2px] text-[10px] text-[#646464]">{accuracy}</p></div>
    </div>
  );
}
...
function QuestionCard({ row, selected, resolved, onAnswer }: { row: Row; selected?: InspectionAnswerValue; resolved: boolean; onAnswer: (value: InspectionAnswerValue) => void }) {
  ...
  return (
    <div className="mb-[10px] ml-[33px] mr-[12px] overflow-hidden rounded-[12px] border border-[#E3E3E3] bg-white">
      <div className="flex items-center justify-between gap-[8px] bg-[#002659] px-[12px] py-[7px] text-white"><p className="min-w-0 flex-1 truncate text-[12px] font-bold">{row.sectionTitle}</p><p className="text-[11px] font-bold">{row.code}</p></div>
      <div className="p-[12px]"><div className="flex items-start gap-[8px]"><span className="flex h-[22px] min-w-[22px] items-center justify-center rounded-full bg-[#001E39] text-[11px] font-bold text-white">{row.index + 1}</span><p className="flex-1 text-[13px] font-semibold leading-[19px] text-[#131313]">{row.question}</p></div>{row.guidance ? <p className="mt-[8px] text-[11px] leading-[16px] text-[#646464]">{row.guidance}</p> : null}<div className="mt-[10px] flex gap-[8px]">{options.map((option) => { const active = selected === option.value; return <button key={option.value} type="button" disabled={resolved || Boolean(selected)} onClick={() => onAnswer(option.value)} className={`h-[36px] flex-1 rounded-full border-[1.5px] text-[13px] font-bold ${active ? 'border-[#002659] bg-[#002659] text-white' : 'border-[#D1D1D1] bg-white text-[#24588B] disabled:opacity-45'}`}>{option.label}</button>; })}</div></div>
    </div>
  );
}
...
export function AssistantChatStep({ onBack, onSave, onCancelInspection, saving, errorMessage }: AssistantChatStepProps) {
  ...
  function push(type: MsgType, data?: unknown) {
    sequence.current += 1;
    const message = { id: String(sequence.current), t: type, data };
  ...
  function startFresh() {
    ...
    useNewInspectionDraftStore.getState().reset();
    setMessages([]);
  ...
  async function beginAreaSelection() {
    push('typing');
    try {
      const areas = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', 'areas'], queryFn: getOrganizationAreas, staleTime: 300000 });
      clearTyping();
      push('bot', 'Hola, soy AurelIA. ¿En qué área estás hoy?');
      push('areas', areas);
    } catch {
      clearTyping();
      pushError('No pude cargar áreas.', () => { void beginAreaSelection(); });
    }
  }
  ...
  async function beginSectorSelection(areaId: string) {
    push('typing');
    try {
      const sectors = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', 'sectors', areaId], queryFn: () => getOrganizationSectors(areaId), staleTime: 300000 });
      clearTyping();
      push('bot', 'Selecciona el sector.');
      push('sectors', sectors);
    } catch {
      clearTyping();
      pushError('No pude cargar sectores.', () => { void beginSectorSelection(areaId); });
    }
  }
  ...
  async function selectSector(sector: SectorResponse, messageId: string) {
    ...
    push('typing');
    try {
      const types = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', 'inspection-types'], queryFn: getInspectionTypes, staleTime: 300000 });
      clearTyping();
      setStep(0);
      push('bot', 'Selecciona el tipo de inspección.');
      push('types', [{ value: InspectionType.ENVIRONMENTAL, label: 'Hallazgo', icon: 'search', inspectionTypeId: types.find((item) => item.code === InspectionType.ENVIRONMENTAL)?.id ?? null }, { value: InspectionType.REGULATORY, label: 'Checklist normativo', icon: 'clipboard-check', inspectionTypeId: checklistTypeId(types) }] as TypeOption[]);
    } catch {
      clearTyping();
      pushError('No pude cargar tipos.', () => { void selectSector(sector, messageId); });
    }
  }
  ...
  async function captureChatLocation(messageId: string) {
    ...
    if (!ok) {
      pushError(locationError ?? 'No se pudo capturar la ubicación.', () => { void captureChatLocation(messageId); });
      return;
    }
    markResolved(messageId);
    push('user', `Ubicación capturada · ${useNewInspectionDraftStore.getState().locationAccuracyLabel}`);
    if (useNewInspectionDraftStore.getState().inspectionType === InspectionType.ENVIRONMENTAL) {
  ...
  async function askChecklistTemplate() {
    ...
    try {
      ...
      clearTyping();
      if (!templates.length) {
        pushError('No hay plantillas normativas disponibles.');
        return;
      }
      setStep(1);
      push('bot', 'Te sugiero esta plantilla normativa.');
      push('templatePick', { template: templates[0], templates } as TemplatePickData);
    } catch {
      clearTyping();
      pushError('No pude cargar plantillas normativas.', () => { void askChecklistTemplate(); });
    }
  }
  ...
  async function selectChecklistTemplate(template: InspectionChecklistTemplateResponse, messageId: string) {
    ...
    draft.setTemplate({ id: template.id, name: template.name, code: template.code, itemsCount: rows.length });
  ...
  function openChecklistTemplates(templates: InspectionChecklistTemplateResponse[], messageId: string) {
    markResolved(messageId);
    push('bot', 'Elige una plantilla.');
    push('templates', templates);
  }

  async function handleChecklistGeneralPhoto(asset: NewInspectionPickedAsset, messageId: string) {
    setPhotoReceiptByMessageId((prev) => ({ ...prev, [messageId]: { title: 'Foto adjunta ✓', sub: receiptText(asset.name) } }));
    markResolved(messageId);
    draft.setGeneralPhoto(asset);
    ...
    setStep(1);
    push('bot', `Responderemos ${checklistRows.length} ítems.`);
    push('question', first);
  }
  ...
  async function handleChecklistItemPhoto(asset: NewInspectionPickedAsset, row: Row, messageId: string) {
    setPhotoReceiptByMessageId((prev) => ({ ...prev, [messageId]: { title: 'Foto adjunta ✓', sub: receiptText(asset.name) } }));
    markResolved(messageId);
    draft.setItemDetail(row.id, { evidence: asset });
  ...
  async function goToNextChecklistQuestion(index: number) {
    await sleep(200);
    const next = checklistRows[index + 1];
    if (next) {
      push('question', next);
      return;
    }
    await finishChecklistItems(checklistRows);
  }
  ...
  async function finishChecklistItems(rows: Row[]) {
    ...
    await askCompany('responsible-companies-checklist');
  }

  async function askFindingType() {
    push('typing');
    try {
      const findingTypes = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', 'finding-types'], queryFn: getInspectionFindingTypes, staleTime: 300000 });
      clearTyping();
      ...
      clearTyping();
      pushError('No pude cargar tipos de hallazgo.', () => { void askFindingType(); });
    }
  }
  ...
  async function handleFindingCondition(text: string, observationId: string) {
    ...
    push('findingPhoto', { observationId } as FindingPhotoData);
    setWaiting(null);
  }

  async function handleFindingPhoto(asset: NewInspectionPickedAsset, observationId: string, messageId: string) {
    setPhotoReceiptByMessageId((prev) => ({ ...prev, [messageId]: { title: 'Foto adjunta ✓', sub: receiptText(asset.name) } }));
    markResolved(messageId);
    draft.updateFindingObservation(observationId, { evidence: asset });
    push('user', 'Foto del hallazgo registrada');
    await sleep(200);
    push('bot', 'Escribe o confirma la medida correctiva.');
    setWaiting(`finding-measure:${observationId}`);
  }
  ...
  async function askFindingCriticality(observationId: string) {
    ...
    push('typing');
    try {
      const severities = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', 'finding-severities'], queryFn: getInspectionFindingSeverities, staleTime: 300000 });
      clearTyping();
      ...
      clearTyping();
      pushError('No pude cargar criticidades.', () => { void askFindingCriticality(observationId); });
    }
  }
  ...
  async function completeFindingCriticality(observationId: string, severity: InspectionFindingSeverityResponse, messageId: string) {
    markResolved(messageId);
    draft.updateFindingObservation(observationId, { severityId: severity.id, severityLabel: severity.name, severityClosureTimeLabel: severity.closureTimeLabel });
    await sleep(200);
    push('bot', 'Confirma el SLA para esta observación.');
    push('sla', { observationId, initialDays: parseSlaDays(severity.closureTimeLabel, 7) } as SlaData);
  }
  ...
  async function saveFindingObservation(messageId: string, data: SlaData, days: number) {
    ...
    draft.updateFindingObservation(data.observationId, { severityClosureTimeLabel: defaultSlaLabel(days), saved: true });
    push('user', `✓ Observación guardada (SLA ${days} días)`);
    await sleep(200);
    setStep(3);
    push('bot', '¿Deseas agregar otra observación o continuar con empresa y personal?');
    push('findingNext', { observationId: data.observationId });
  }
  ...
  async function findingDecision(value: 'add' | 'continue', messageId: string) {
    ...
    await askCompany('responsible-companies-finding');
  }

  async function askCompany(queryKey: string) {
    push('typing');
    try {
      const companies = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', queryKey], queryFn: getResponsibleCompanies, staleTime: 300000 });
      clearTyping();
      if (!companies.length) {
      ...
      clearTyping();
      pushError('No pude cargar empresas.', () => { void askCompany(queryKey); });
    }
  }
  ...
  async function askResponsiblePeople(companyId: string) {
    push('typing');
    try {
      const users = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', 'responsibles', companyId], queryFn: () => getCompanyUsers(companyId), staleTime: 300000 });
      clearTyping();
      if (!users.length) {
        pushError('No hay personal asociado a la empresa seleccionada. Elige otra empresa.');
        const companies = await getResponsibleCompanies();
        push('bot', 'Selecciona otra empresa para continuar.');
        push('companies', companies);
        return;
      }
      const suggested = suggestedResponsible(users);
      const state = useNewInspectionDraftStore.getState();
      ...
      clearTyping();
      pushError('No pude cargar personal.', () => { void askResponsiblePeople(companyId); });
    }
  }
  ...
  function renderMessage(message: Msg) {
    ...
    if (message.t === 'bot') return <BotBubble key={message.id}><p className="text-[13px] leading-[18px] text-[#131313]">{String(message.data ?? '')}</p></BotBubble>;
    ...
    if (message.t === 'areas') {
      ...
      return <ChipRow key={message.id} chips={areas.map((item) => item.name)} selected={resolved ? draft.areaName : null} onSelect={(name) => { if (resolved) return; const area = areas.find((item) => item.name === name); if (area) void selectArea(area, message.id); }} />;
    }
    if (message.t === 'sectors') {
      const sectors = message.data as SectorResponse[];
      return <ChipRow key={message.id} chips={sectors.map((item) => item.name)} selected={resolved ? draft.sectorName : null} onSelect={(name) => { if (resolved) return; const sector = sectors.find((item) => item.name === name); if (sector) void selectSector(sector, message.id); }} />;
    }
    if (message.t === 'types') {
      const options = message.data as TypeOption[];
      return <QuickOpts key={message.id} options={options.map((item) => ({ value: item.value, label: item.label, icon: item.icon }))} selected={resolved ? draft.inspectionType : null} onSelect={(value) => { if (resolved) return; const option = options.find((item) => item.value === value); if (option) void selectInspectionType(option.value, option.inspectionTypeId, option.label, message.id); }} />;
    }
    ...
    if (message.t === 'templatePick') {
      const data = message.data as TemplatePickData;
    ...
    if (message.t === 'templates') {
      const templates = message.data as InspectionChecklistTemplateResponse[];
      return <ChipRow key={message.id} chips={templates.map((item) => item.name)} selected={resolved ? draft.templateName : null} variant="navy" onSelect={(name) => { if (resolved) return; const template = templates.find((item) => item.name === name); if (template) void selectChecklistTemplate(template, message.id); }} />;
    }
    if (message.t === 'generalPhoto') return <PhotoStepWidget key={message.id} resolved={resolved} receipt={photoReceiptByMessageId[message.id]} onCapture={(asset) => { if (!resolved) void handleChecklistGeneralPhoto(asset, message.id); }} onSkip={() => pushError('La foto general es obligatoria para checklist normativo.')} />;
    if (message.t === 'question') {
      const row = message.data as Row;
      return <QuestionCard key={message.id} row={row} selected={draft.answersByItemId[row.id]} resolved={resolved} onAnswer={(value) => { if (!resolved) void answerChecklistItem(row, value, message.id); }} />;
    }
    if (message.t === 'itemPhoto') {
      const row = message.data as Row;
      return <PhotoStepWidget key={message.id} resolved={resolved} receipt={photoReceiptByMessageId[message.id]} onCapture={(asset) => { if (!resolved) void handleChecklistItemPhoto(asset, row, message.id); }} onSkip={() => pushError('La evidencia fotográfica es obligatoria para ítems NO conformes.')} />;
    }
    if (message.t === 'findingTypes') {
      const types = message.data as InspectionFindingTypeResponse[];
      return <ChipRow key={message.id} chips={types.map((item) => item.name)} selected={resolved ? draft.findingTypeLabel : null} variant="navy" onSelect={(name) => { if (resolved) return; const found = types.find((item) => item.name === name); if (found) void selectFindingType(found, message.id); }} />;
    }
    if (message.t === 'findingPhoto') {
      const data = message.data as FindingPhotoData;
      return <PhotoStepWidget key={message.id} resolved={resolved} receipt={photoReceiptByMessageId[message.id]} onCapture={(asset) => { if (!resolved) void handleFindingPhoto(asset, data.observationId, message.id); }} onSkip={() => pushError('Para registrar un hallazgo en chat, la evidencia fotográfica es obligatoria.')} />;
    }
    if (message.t === 'criticality') {
      const data = message.data as CriticalityData;
      return <CriticalityCard key={message.id} severities={data.severities} resolved={resolved} onSelect={(severity) => { if (!resolved) void completeFindingCriticality(data.observationId, severity, message.id); }} />;
    }
    if (message.t === 'sla') {
      const data = message.data as SlaData;
      return <SlaConfirmWidget key={message.id} initialDays={data.initialDays} resolved={resolved} onSave={(days) => { if (!resolved) void saveFindingObservation(message.id, data, days); }} />;
    }
    ...
    if (message.t === 'companies') {
      ...
      return <ChipRow key={message.id} chips={companies.map((item) => item.name)} selected={resolved ? draft.findingCompanyName : null} onSelect={(name) => { if (resolved) return; const company = companies.find((item) => item.name === name); if (company) void selectCompany(company, message.id); }} />;
    }
    if (message.t === 'people') {
      const data = message.data as PeopleData;
      return <PersonnelPicker key={message.id} users={data.users} suggestedUserId={data.suggestedUserId} confirmed={confirmedPeople.has(message.id)} onConfirm={(selected) => { void confirmPeople(selected, message.id); }} />;
    }
  ...
  return (
    <>
      <div className="bg-[#002659] text-white">
        <div className="flex h-[56px] items-center justify-between gap-[8px] px-[8px]"><button type="button" onClick={onBack} className="flex h-[44px] w-[32px] items-center justify-center rounded-full text-[18px] text-[rgba(255,255,255,0.75)]">←</button><div className="flex flex-1 items-center gap-[8px]"><div className="relative"><div className="flex h-[40px] w-[40px] items-center justify-center rounded-full bg-[#C8A064] text-[#001E39]">✦</div><div className="absolute bottom-[1px] right-[1px] h-[10px] w-[10px] rounded-full border-[2px] border-[#002659] bg-[#00B398]" /></div><div><p className="text-[14px] font-semibold text-white">AurelIA</p><div className="mt-[1px] flex items-center gap-[4px]"><span className="h-[6px] w-[6px] rounded-full bg-[#00B398]" /><p className="text-[12px] text-[rgba(255,255,255,0.55)]">{saving ? 'Pensando' : 'Activo'}</p></div></div></div><button type="button" onClick={onCancelInspection} className="flex h-[48px] w-[48px] items-center justify-center rounded-full text-[15px] text-[rgba(255,255,255,0.6)]">⋮</button></div>
        <div className="border-b border-[rgba(255,255,255,0.06)] px-[16px] pb-[7px]"><div className="mb-[5px] flex gap-[3px]">{STEP_LABELS.map((_, index) => <div key={`assistant-step-dot-${index}`} className={`h-[3px] flex-1 rounded ${index < safeStep ? 'bg-[#C8A064]' : index === safeStep ? 'bg-[rgba(200,160,100,0.5)]' : 'bg-[rgba(255,255,255,0.22)]'}`} />)}</div><div className="flex items-center justify-between text-white"><p className="text-[10px] text-[rgba(255,255,255,0.45)]"><span className="font-semibold text-[rgba(255,255,255,0.7)]">Paso {safeStep + 1} · {STEP_LABELS[safeStep]}</span></p><p className="text-[10px] text-[rgba(255,255,255,0.45)]">{STEP_PCT[safeStep]}</p></div></div>
      </div>
      <div ref={scrollRef} className="flex-1 overflow-y-auto bg-[#D1D4DA] px-[10px] pb-[16px] pt-[12px]">{messages.map(renderMessage)}</div>
      <ChatInput disabled={waiting === null || saving} value={inputValue} onChange={setInputValue} onSend={sendText} />
    </>
  );
}
