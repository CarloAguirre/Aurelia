...
type MsgType = 'bot' | 'user' | 'typing' | 'error' | 'areas' | 'sectors' | 'types' | 'dates' | 'loc' | 'templatePick' | 'templates' | 'generalPhoto' | 'question' | 'itemPhoto' | 'findingTypes' | 'findingPhoto' | 'aiMeasure' | 'criticality' | 'sla' | 'findingNext' | 'companies' | 'people' | 'summary';
...
function BotBubble({ children }: { children: ReactNode }) {
  return <div className="mb-[10px] flex w-full items-end gap-[7px]"><div className="flex h-[26px] w-[26px] shrink-0 items-center justify-center rounded-full bg-[#CAA262] text-[11px] text-[#001E39]">✦</div><div className="max-w-[85%] rounded-[16px] rounded-bl-[4px] border border-[#E3E3E3] bg-white px-[12px] py-[10px] shadow-[0_1px_3px_rgba(0,0,0,0.06)]"><div>{children}</div><p className="mt-[6px] text-[11px] text-[#99A0AF]">{new Date().toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit' })}</p></div></div>;
}

function UserBubble({ children }: { children: ReactNode }) {
  return <div className="mb-[10px] ml-auto max-w-[78%] rounded-[16px] rounded-br-[4px] bg-[#002659] px-[12px] py-[10px]"><div className="text-[13px] font-medium leading-[18px] text-white">{children}</div><p className="mt-[6px] text-[11px] text-[rgba(255,255,255,0.38)]">{new Date().toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit' })}</p></div>;
}

function TypingBubble() {
  return <div className="mb-[10px] flex w-full items-end gap-[7px]"><div className="flex h-[26px] w-[26px] shrink-0 items-center justify-center rounded-full bg-[#CAA262] text-[11px] text-[#001E39]">✦</div><div className="inline-flex items-center gap-[4px] rounded-[14px] rounded-bl-[4px] border border-[#D3D7DE] bg-white px-[14px] py-[10px]"><span className="h-[6px] w-[6px] animate-bounce rounded-full bg-[#9CA3AF]" /><span className="h-[6px] w-[6px] animate-bounce rounded-full bg-[#9CA3AF] [animation-delay:150ms]" /><span className="h-[6px] w-[6px] animate-bounce rounded-full bg-[#9CA3AF] [animation-delay:300ms]" /></div></div>;
}
...
function ChipRow({ chips, selected, onSelect, variant = 'gold' }: { chips: string[]; selected?: string | null; onSelect: (label: string) => void; variant?: 'gold' | 'navy' }) {
  return <div className="mb-[10px] ml-[33px] flex flex-wrap gap-[8px]">{chips.map((chip) => { const active = selected === chip; const activeClass = variant === 'navy' ? 'border-[#052B63] bg-[#052B63] text-white' : 'border-[#C8A064] bg-[#C8A064] text-[#052B63]'; return <button key={chip} type="button" onClick={() => onSelect(chip)} className={`rounded-full border-[1.5px] px-[12px] py-[6px] text-[11px] font-semibold ${active ? activeClass : 'border-[#D1D1D1] bg-white text-[#646464]'}`}>{chip}</button>; })}</div>;
}
...
function PhotoStepWidget({ resolved, receipt, onCapture, onSkip }: { resolved: boolean; receipt?: PhotoReceipt; onCapture: (asset: NewInspectionPickedAsset) => void; onSkip: () => void }) {
  ...
  return <div className="mb-[10px] ml-[33px] rounded-[12px] border border-[#E3E3E3] bg-white p-[12px]">{resolved && receipt ? <div className="flex min-h-[58px] items-center gap-[10px] rounded-[10px] bg-[#35A137] px-[12px] py-[8px] text-white"><span className="flex h-[42px] w-[42px] items-center justify-center rounded-[8px] bg-[rgba(255,255,255,0.24)]">📷</span><span className="min-w-0"><span className="block truncate text-[13px] font-bold">{receipt.title}</span><span className="block truncate text-[11px] text-[rgba(255,255,255,0.78)]">{receipt.sub}</span></span></div> : <div className="grid gap-[8px]"><label className="flex min-h-[84px] cursor-pointer items-center rounded-[10px] border-[1.5px] border-dashed border-[#D1D1D1] bg-[#F6FAFF] px-[12px] py-[10px]"><input type="file" className="hidden" accept="image/*" disabled={resolved} onChange={onFile} /><span className="flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-[8px] bg-white text-[16px]">📷</span><span className="ml-[10px] flex min-w-0 flex-col"><span className="truncate text-[13px] font-semibold text-[#646464]">Tomar foto o galería</span><span className="mt-[2px] text-[11px] text-[#B7B7B7]">Fecha, hora y GPS automáticos</span></span></label><button type="button" className="h-[34px] rounded-[10px] border border-[#E3E3E3] text-[12px] font-bold text-[#646464]" onClick={onSkip}>Omitir</button></div>}</div>;
}
...
export function AssistantChatStep({ onBack, onSave, onCancelInspection, saving, errorMessage }: AssistantChatStepProps) {
  ...
  useEffect(() => { if (booted.current) return; booted.current = true; startFresh(); }, []);

  function startFresh() { retries.current.clear(); useNewInspectionDraftStore.getState().reset(); setMessages([]); setResolvedMessages(new Set()); setConfirmedPeople(new Set()); setPhotoReceiptByMessageId({}); setWaiting(null); setInputValue(''); setChecklistRows([]); setStep(0); void beginAreaSelection(); }
  ...
  async function askCompany(queryKey: string) { push('typing'); try { const companies = await queryClient.fetchQuery({ queryKey: ['inspections', 'assistant-chat', queryKey], queryFn: getResponsibleCompanies, staleTime: 300000 }); clearTyping(); if (!companies.length) { pushError('No hay empresas contratistas disponibles en catálogos.'); return; } setStep(4); push('bot', 'Selecciona empresa responsable de los hallazgos.'); push('companies', companies); } catch { clearTyping(); pushError('No pude cargar empresas.', () => { void askCompany(queryKey); }); } }
  ...
  function renderMessage(message: Msg) {
    ...
    if (message.t === 'itemPhoto') { const row = message.data as Row; return <PhotoStepWidget key={message.id} resolved={resolved} receipt={photoReceiptByMessageId[message.id]} onCapture={(asset) => { if (!resolved) void handleChecklistItemPhoto(asset, row, message.id); }} onSkip={() => pushError('La evidencia fotográfica es obligatoria para ítems NO conformes.')} />; }
    ...
    if (message.t === 'findingPhoto') { const data = message.data as FindingPhotoData; return <PhotoStepWidget key={message.id} resolved={resolved} receipt={photoReceiptByMessageId[message.id]} onCapture={(asset) => { if (!resolved) void handleFindingPhoto(asset, data.observationId, message.id); }} onSkip={() => pushError('Para registrar un hallazgo en chat, la evidencia fotográfica es obligatoria.')} />; }
