...
export function AssistantChatStep(props: AssistantChatStepProps) {
  ...
  const observation = draft.findingObservations.find((item) => !item.saved) ?? draft.findingObservations[draft.findingObservations.length - 1];
  const shouldResumeEvidence = props.resumeFromDraft && draft.flowMode !== 'manual' && Boolean(draft.findingTypeId && observation?.detectedCondition && !observation.evidence && !doneEvidenceResume);

  if (shouldResumeEvidence) return <EvidenceResume props={props} onDone={() => setDoneEvidenceResume(true)} />;
