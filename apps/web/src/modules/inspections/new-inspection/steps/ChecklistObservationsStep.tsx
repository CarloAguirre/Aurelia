...
import { getCompanyUsers, getResponsibleCompanies, getInspectionTemplates } from '../../../../shared/services/inspections.service';
...
function templateHeader(template: InspectionChecklistTemplateResponse | undefined, fallback: string | null) {
...
function HashIcon() {
  return <svg width="14" height="12" viewBox="0 0 14 12" fill="none" aria-hidden="true"><path d="M4.6 1.4 3.4 10.6M10.6 1.4 9.4 10.6M1.8 4.2h10.8M1.4 7.8h10.8" stroke="#646464" strokeWidth="1.2" strokeLinecap="round" /></svg>;
}

function ListIcon() {
  return <svg width="14" height="12" viewBox="0 0 14 12" fill="none" aria-hidden="true"><path d="M2 3h1M5 3h7M2 6h1M5 6h7M2 9h1M5 9h7" stroke="#646464" strokeWidth="1.4" strokeLinecap="round" /></svg>;
}
...
function AttachmentInput({ value, emptyLabel, emptySubtitle, photo, onPick }: { value: string | null; emptyLabel: string; emptySubtitle?: string; photo?: boolean; onPick: (name: string, file: File) => void }) {
...
function ChecklistItemCard({ item, index, answer, detail, onAnswer, onDetail }: { item: ChecklistItemRow; index: number; answer: InspectionAnswerValue | undefined; detail: NewInspectionChecklistItemDetail; onAnswer: (value: InspectionAnswerValue) => void; onDetail: (patch: Partial<NewInspectionChecklistItemDetail>) => void }) {
  ...
  return (
    <div className="border-b border-[#E3E3E3] pb-px last:border-b-0">
      <div className="flex items-start px-[12px] pb-[6px] pt-[11px]"><span className="shrink-0 pt-px text-[10px] font-bold leading-none text-[#ACACAC]">{index + 1}</span><p className="ml-[2px] flex-1 text-[12px] leading-[18px] text-[#131313]">{item.question}</p></div>
      <div className="flex w-full gap-[6px] pb-[10px] pl-[32px] pr-[12px]"><AnswerButton label="SÍ" tone="yes" selected={answer === InspectionAnswerValue.COMPLIANT} onPress={() => onAnswer(InspectionAnswerValue.COMPLIANT)} /><AnswerButton label="NO" tone="no" selected={answer === InspectionAnswerValue.NOT_COMPLIANT} onPress={() => onAnswer(InspectionAnswerValue.NOT_COMPLIANT)} /><AnswerButton label="N/A" tone="na" selected={answer === InspectionAnswerValue.NOT_APPLICABLE} onPress={() => onAnswer(InspectionAnswerValue.NOT_APPLICABLE)} /></div>
      {isNo ? <div className="mb-[10px] ml-[32px] mr-[12px] grid gap-[8px] rounded-[10px] border border-[#E3E3E3] bg-[#FAFAFA] p-[10px]"><div><p className="text-[12px] font-bold text-[#131313]">Condición detectada *</p><textarea className="mt-[4px] min-h-[64px] w-full resize-none rounded-[10px] border-[1.5px] border-[#D1D1D1] bg-[#F6FAFF] px-[10px] py-[8px] text-[13px] text-[#131313]" value={detail.detectedCondition ?? ''} onChange={(event) => onDetail({ detectedCondition: event.target.value })} placeholder="Describe la condición detectada" /></div><div><p className="text-[12px] font-bold text-[#131313]">Medida correctiva propuesta *</p><textarea className="mt-[4px] min-h-[64px] w-full resize-none rounded-[10px] border-[1.5px] border-[#D1D1D1] bg-[#F6FAFF] px-[10px] py-[8px] text-[13px] text-[#131313]" value={detail.correctiveAction ?? ''} onChange={(event) => onDetail({ correctiveAction: event.target.value })} placeholder="Indique la medida correctiva" /></div><div><p className="text-[12px] font-bold text-[#131313]">Evidencia *</p><AttachmentInput value={detail.evidence?.name ?? null} emptyLabel="Adjuntar foto" onPick={(name, file) => onDetail({ evidence: { name, file } })} /></div></div> : null}
      {isYes ? <div className="mb-[10px] ml-[32px] mr-[12px]"><p className="text-[12px] font-bold text-[#131313]">Comentario (Opcional)</p><textarea className="mt-[4px] min-h-[54px] w-full resize-none rounded-[10px] border-[1.5px] border-[#D1D1D1] bg-[#F6FAFF] px-[10px] py-[8px] text-[13px] text-[#131313]" value={detail.comment ?? ''} onChange={(event) => onDetail({ comment: event.target.value })} placeholder="Comentario" /></div> : null}
    </div>
  );
}
...
export function ChecklistObservationsStep({ onBack, onNext }: ChecklistObservationsStepProps) {
  ...
  const templateTitle = templateHeader(selectedTemplate, draft.templateName);
