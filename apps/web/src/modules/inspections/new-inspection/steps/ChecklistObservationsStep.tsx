...
import { useNewInspectionDraftStore, type NewInspectionChecklistItemDetail, type NewInspectionDraft } from '../state/newInspectionDraft.store';
...
function MiniIcon({ children }: { children: string }) {
  return <span className="flex h-[20px] w-[20px] items-center justify-center rounded-full text-[13px] leading-none">{children}</span>;
}
...
function ManualStepper() {
  const steps = [{ label: 'Datos', complete: true }, { label: 'Tipo', complete: true }, { label: 'Ítems', active: true }, { label: 'Resumen' }];
  return (
...
function UploadBox({ value, label, subtitle, onPick }: { value: string | null; label: string; subtitle?: string; onPick: (name: string, file: File) => void }) {
  ...
  if (value) {
    return <label className="flex min-h-[60px] w-full cursor-pointer items-center gap-[8px] rounded-[8px] bg-[#3A9B3A] px-[12px] py-[10px] text-white"><input type="file" className="hidden" accept="image/*" onChange={onChange} /><span className="flex h-[40px] w-[40px] shrink-0 items-center justify-center rounded-[8px] bg-[rgba(255,255,255,0.25)]"><MiniIcon>📷</MiniIcon></span><span className="truncate text-[12px] font-bold leading-none">{value}</span></label>;
  }
...
function SummaryBox({ detail, severity, onDelete }: { detail: NewInspectionChecklistItemDetail; severity?: SeverityChoice; onDelete: () => void }) {
  return <div className="mb-[10px] ml-[32px] mr-[12px] rounded-[10px] border-[1.5px] border-[#E3E3E3] bg-white p-[13.5px] shadow-[0_1px_1.5px_rgba(0,0,0,0.06)]"><div className="flex items-center justify-between"><div className="flex items-center gap-[8px]"><span className="rounded-[6px] bg-[#E6F3FF] px-[8px] py-[4px] text-[11px] font-bold leading-none text-[#24588B]">Obs. 2</span>{severity ? <span className="rounded-[8px] bg-[#FFE1CD] px-[8px] py-[5px] text-[10px] font-bold leading-none text-[#532A0E]">{severity.label}</span> : null}</div><button type="button" onClick={onDelete} className="flex h-[28px] w-[28px] items-center justify-center rounded-[6px] bg-[#FFD0DB] text-[#8A0F2E]">🗑</button></div><div className="mt-[12px] grid gap-[4px]"><div className="rounded-[8px] border border-[#E3E3E3] bg-white px-[11px] py-[9px]"><p className="text-[9px] font-bold uppercase tracking-[1.5px] text-[#646464]">Condición detectada</p><p className="mt-[3px] text-[12px] leading-[16.8px] text-[#131313]">{detail.detectedCondition?.trim() || 'Sin descripción'}</p></div><div className="rounded-[8px] bg-[#F7F7F7] px-[10px] py-[8px]"><p className="text-[9px] font-bold uppercase tracking-[1.5px] text-[#646464]">Medida correctiva propuesta</p><p className="mt-[3px] text-[12px] leading-[16.8px] text-[#131313]">{detail.correctiveAction?.trim() || 'Sin medida registrada'}</p></div>{detail.evidence?.name ? <div className="border-t-[1.5px] border-[#C8A064] bg-[#FFFDF7] px-[10px] pb-[8px] pt-[9.5px]"><UploadBox value={detail.evidence.name} label="Tomar foto o galería" onPick={() => undefined} /></div> : null}<div className="flex items-center justify-between border-t border-[#E3E3E3] px-[12px] pb-[9px] pt-[10px]"><span className="text-[12px] font-medium text-[#646464]">SLA calculado</span><span className="text-right text-[12px] font-bold text-[#131313]">{severity?.closureTimeLabel ?? 'xx días hábiles'}</span></div></div></div>;
}
...
function ChecklistItemCard({ item, index, answer, detail, severity, severities, editing, onAnswer, onDetail, onAddRecords, onCancelRecords, onSaveRecords, onDeleteRecords, onOpenSeverity }: { item: ChecklistItemRow; index: number; answer: InspectionAnswerValue | undefined; detail: NewInspectionChecklistItemDetail; severity?: SeverityChoice; severities: SelectSheetOption[]; editing: boolean; onAnswer: (value: InspectionAnswerValue) => void; onDetail: (patch: Partial<NewInspectionChecklistItemDetail>) => void; onAddRecords: () => void; onCancelRecords: () => void; onSaveRecords: () => void; onDeleteRecords: () => void; onOpenSeverity: () => void }) {
...
function ChecklistItemsCard({ title, code, items, draft, severities, severityByItemId, editingItemId, setAnswer, setItemDetail, setEditingItemId, setSeverityByItemId, openSeverityPicker }: { title: string; code: string; items: ChecklistItemRow[]; draft: NewInspectionDraft; severities: SelectSheetOption[]; severityByItemId: SeverityByItemId; editingItemId: string | null; setAnswer: (itemId: string, value: InspectionAnswerValue) => void; setItemDetail: (itemId: string, detail: Partial<NewInspectionChecklistItemDetail>) => void; setEditingItemId: (id: string | null) => void; setSeverityByItemId: (updater: (value: SeverityByItemId) => SeverityByItemId) => void; openSeverityPicker: (id: string) => void }) {
  return <div className="min-h-px w-full overflow-hidden rounded-[12px] border border-[#E3E3E3] bg-white shadow-[0_1px_3px_rgba(0,0,0,0.05)]"><div className="flex items-center justify-between bg-[#001E39] px-[14px] py-[10px]"><p className="truncate text-[12px] font-bold leading-none text-white">{title}</p><p className="ml-[10px] shrink-0 text-[10px] font-normal leading-none text-[rgba(255,255,255,0.45)]">{code}</p></div>{items.length === 0 ? <p className="px-[12px] py-[14px] text-[12px] text-[#646464]">Esta plantilla no tiene ítems activos.</p> : null}{items.map((item, index) => <ChecklistItemCard key={item.id} item={item} index={index} answer={draft.answersByItemId[item.id]} detail={draft.detailsByItemId[item.id] ?? {}} severity={severityByItemId[item.id]} severities={severities} editing={editingItemId === item.id} onAnswer={(value) => { setAnswer(item.id, value); if (value === InspectionAnswerValue.NOT_COMPLIANT) setEditingItemId(null); }} onDetail={(patch) => setItemDetail(item.id, patch)} onAddRecords={() => setEditingItemId(item.id)} onCancelRecords={() => setEditingItemId(null)} onSaveRecords={() => setEditingItemId(null)} onDeleteRecords={() => { setItemDetail(item.id, { correctiveAction: '', evidence: null }); setSeverityByItemId((current) => ({ ...current, [item.id]: undefined })); }} onOpenSeverity={() => openSeverityPicker(item.id)} />)}</div>;
}
