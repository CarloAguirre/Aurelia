import { SPR_ACTIVE_CYCLE, SPR_REJECTED_STATUS, SPR_SUBMITTED_STATUS } from '../spr.constants';

export type SprSubmittedSummaryVariant = 'pending_approval' | 'rejected';
...
// Barra resumen post-envio (Figma 1666:2149 / 1672:8919).
export function SprSubmittedSummaryHeader({ signDateLabel, variant = 'pending_approval' }: SprSubmittedSummaryHeaderProps) {
  const statusCopy = variant === 'rejected' ? SPR_REJECTED_STATUS : SPR_SUBMITTED_STATUS;
  const formStatusColor = variant === 'rejected' ? 'text-[#570b1d]' : 'text-[#8e6e3e]';
