import { SPR_ACTIVE_CYCLE, SPR_APPROVED_STATUS, SPR_REJECTED_STATUS, SPR_SUBMITTED_STATUS } from '../spr.constants';

export type SprSubmittedSummaryVariant = 'pending_approval' | 'rejected' | 'completed';
...
export function SprSubmittedSummaryHeader({ signDateLabel, variant = 'pending_approval' }: SprSubmittedSummaryHeaderProps) {
  ...
  const formStatusColor =
    variant === 'rejected' ? 'text-[#570b1d]' : variant === 'completed' ? 'text-[#3a9b3a]' : 'text-[#8e6e3e]';
